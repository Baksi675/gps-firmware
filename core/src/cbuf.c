 ...
 */
  ...
  * @param output_buf The output buffer to read to.
  */
void cbuf_read(CBUF_HANDLE_ts *cbuf_handle, uint8_t* output_buf) {
	while(cbuf_len(cbuf_handle) != 0) {
		*output_buf = cbuf_handle->ptr[cbuf_handle->read_position];
		output_buf++;
...
void cbuf_write(CBUF_HANDLE_ts *cbuf_handle, uint8_t* input_buf, uint32_t input_len) {
	while(input_len != 0) {
		if(cbuf_len(cbuf_handle) == cbuf_handle->size - 1) {
			break;
		}
...
uint8_t cbuf_len(CBUF_HANDLE_ts *cbuf_handle) {	
	return ((cbuf_handle->write_position - cbuf_handle->read_position) & (cbuf_handle->size - 1));
}
