 ...
 * @brief GTU7 GPS module implementation file.
 * @version 0.1
 * @date 2026-02-03
 * 
...
static ERR_te gtu7_calc_checksum(char *msg, uint32_t msg_len, uint8_t *checksum_o);
static ERR_te gtu7_process_msg(uint8_t *msg, uint32_t msg_len);
static ERR_te gtu7_process_rmc(char **tokens);
static ERR_te gtu7_process_vtg(char **tokens);
static ERR_te gtu7_process_gga(char **tokens);
static ERR_te gtu7_process_gsa(char **tokens);
static ERR_te gtu7_process_gsv(char **tokens);

static ERR_te gtu7_dumpnmea_handler(uint32_t argc, char **argv);

static CMD_INFO_ts gtu7_cmds[] = {
	{
		.name = "dumpnmea",
		.help = "Dumps raw NMEA messages, usage: gtu7 dumpnmea <true|false>",
		.handler = gtu7_dumpnmea_handler
	},
};

static CMD_CLIENT_INFO_ts gtu7_cmd_client_info = {
	.cmds_ptr = gtu7_cmds,
	.num_cmds = sizeof(gtu7_cmds) / sizeof(gtu7_cmds[0]),
	.name = "gtu7",
...
/** 
 * @defgroup gtu7_Public_APIs GPS Public APIs
 * @{
 */

/**
 * @brief Initializes the GTU7 subsystem internal state to a clean state and registers the subsystem commands.
 * 
...
ERR_te gtu7_init_subsys(void) {
	...
	if(internal_state.initialized || internal_state.started) {
		LOG_ERROR(
			...
			"gtu7_init_subsys: subsys is already initialized or started"
		);
	...
	internal_state.subsys = MODULES_GTU7;
	...
	err = cmd_register(&gtu7_cmd_client_info);
	if(err != ERR_OK) {
		LOG_ERROR(
			internal_state.subsys, 
			internal_state.log_level,
			"gtu7_init_subsys: cmd_register error"
		);
	...
	LOG_INFO(
		internal_state.subsys, 
		internal_state.log_level,
		"gtu7_init_subsys: subsys initialized"
	);
...
/**
 * @brief Deinitializes the gtu7 subsystem.
 * 
...
ERR_te gtu7_deinit_subsys(void) {
	if(internal_state.initialized && !internal_state.started) {
		...
		ERR_te err = cmd_deregister(&gtu7_cmd_client_info);
		if(err != ERR_OK) {
			return err;
		}
	}
		...
		LOG_ERROR(
			internal_state.subsys, 
			internal_state.log_level,
			"gtu7_deinit_subsys: subsys is not initialized or stopped"
		);
	...
	LOG_INFO(
		internal_state.subsys, 
		internal_state.log_level,
		"gtu7_deinit_subsys: subsystem deinitialized"
	);
	return ERR_OK;
}
...
ERR_te gtu7_start_subsys(void) {
	if(internal_state.initialized && !internal_state.started) {
		...
		LOG_INFO(
			internal_state.subsys, 
			internal_state.log_level,
			"gtu7_start_subsys: subsys started"
		);
	}
	else {
		LOG_ERROR(
			internal_state.subsys, 
			internal_state.log_level,
			"gtu7_start_subsys: subsys not initialized or already started"
		);
...
ERR_te gtu7_stop_subsys(void) {
	if(internal_state.initialized && internal_state.started) {
		...
		LOG_INFO(
			internal_state.subsys, 
			internal_state.log_level,
			"gtu7_stop_subsys: subsys stopped"
		);
	}
	else {
		LOG_ERROR(
			internal_state.subsys, 
			internal_state.log_level,
			"gtu7_stop_subsys: subsys not initialized or already already stopped"
		);
 ...
 * 
 * @param[in] gtu7_config GPS configuration object.
 * @param[out] gtu7_handle_o Initialized handle.
 * @return ERR_te Error generated during execution.
 */
ERR_te gtu7_init_handle(GTU7_CONFIG_ts *gtu7_config,  GTU7_HANDLE_ts **gtu7_handle_o) {
	if(internal_state.gtu7_handle.initialized == true) {
		LOG_ERROR(
			internal_state.subsys,
			internal_state.log_level,
			"gtu7_init_handle: handle already initialized"
		);

		return ERR_INITIALIZATION_FAILURE;
	}

	if(gtu7_config->usart_instance == NULL) {
		LOG_ERROR(
			internal_state.subsys, 
			internal_state.log_level,
			"gtu7_init_handle: No USART peripheral given"
		);

		return ERR_INITIALIZATION_FAILURE;
	}
	else if(gtu7_config->rx_gpio_port == NULL || gtu7_config->tx_gpio_port == NULL) {
		LOG_ERROR(
			internal_state.subsys, 
			internal_state.log_level,
			"gtu7_init_handle: No GPIO peripheral given"
		);	
		
		return ERR_INITIALIZATION_FAILURE;
	}

	internal_state.gtu7_handle.usart_instance = gtu7_config->usart_instance;
	internal_state.gtu7_handle.usart_baud_rate = gtu7_config->usart_baud_rate;
	internal_state.gtu7_handle.rx_gpio_port = gtu7_config->rx_gpio_port;
	internal_state.gtu7_handle.tx_gpio_port = gtu7_config->tx_gpio_port;
	internal_state.gtu7_handle.rx_gpio_pin = gtu7_config->rx_gpio_pin;
	internal_state.gtu7_handle.tx_gpio_pin = gtu7_config->tx_gpio_pin;
	internal_state.gtu7_handle.gpio_alternate_function = gtu7_config->gpio_alternate_function;

	USART_HANDLE_ts gtu7_usart = { 0 };
	gtu7_usart.instance = gtu7_config->usart_instance;
	gtu7_usart.baud_rate = gtu7_config->usart_baud_rate;
	gtu7_usart.interrupt_en = USART_INTERRUPT_EN_TRUE;
	usart_init(&gtu7_usart);
	usart_set_reception(gtu7_usart.instance, ENABLE);

	GPIO_HANDLE_ts gtu7_rx = { 0 };
	gtu7_rx.port = gtu7_config->rx_gpio_port;
	gtu7_rx.pin = gtu7_config->rx_gpio_pin;
	gtu7_rx.mode = GPIO_MODE_ALTERNATE_FUNCTION;
	gtu7_rx.alternate_function = gtu7_config->gpio_alternate_function;
	gpio_init(&gtu7_rx);

	GPIO_HANDLE_ts gtu7_tx = { 0 };
	gtu7_tx.port = gtu7_config->tx_gpio_port;
	gtu7_tx.pin = gtu7_config->tx_gpio_pin;
	gtu7_tx.mode = GPIO_MODE_ALTERNATE_FUNCTION;
	gtu7_tx.alternate_function = gtu7_config->gpio_alternate_function;
	gpio_init(&gtu7_tx);

	internal_state.gtu7_handle.initialized = true;
...
/**
 * @brief Runs the gtu7 subsystem as part of the state machine.
 * 
 * @return ERR_te Error generated during execution.
 */
	...
	if(data_len) {
		...
		for(uint32_t i = 0; i < data_len; i++) {
			if(data[i] == '\n') {
				...
				ERR_te err = gtu7_process_msg(nmea_msg, nmea_len);
				if(err != ERR_OK) {
					return err;
				}
			}
...
/**
 * @brief Gets the gtu7 information from the internal state.
 * 
 * @param[out] gtu7_info_o The gtu7 GPS information.
 * @return ERR_te Error generated during execution.
 */
ERR_te gtu7_get_info(GTU7_INFO_ts **gtu7_info_o) {
	*gtu7_info_o = &internal_state.gtu7_info;
...
/** 
 * @defgroup gtu7_Internal_Helper GPS Internal Helpers
 * @{
 */
...
static ERR_te gtu7_process_msg(uint8_t *msg, uint32_t msg_len) {
	...
	err = gtu7_calc_checksum((char*)msg, msg_len, &checksum_calc);
	if(err != ERR_OK) {
		return err;
	}

	char *tokens[CONFIG_GTU7_MAX_TOKENS];
	uint16_t num_tokens = 0;

	if(checksum_recv == checksum_calc) {
		str_tokenize((char*)msg, ",", CONFIG_GTU7_MAX_TOKENS, tokens, &num_tokens);

		if(str_cmp("$GPRMC", tokens[0]) == true) {
			gtu7_process_rmc(tokens);
		}
		else if(str_cmp("$GPVTG", tokens[0]) == true) {
			gtu7_process_vtg(tokens);
		}
		else if(str_cmp("$GPGGA", tokens[0]) == true) {
			gtu7_process_gga(tokens);
		}
		else if(str_cmp("$GPGSA", tokens[0]) == true) {
			gtu7_process_gsa(tokens);
		}
		else if(str_cmp("$GPGSV", tokens[0]) == true) {
			gtu7_process_gsv(tokens);
		}
	}
	else {
		// Invalid checksum
...
static ERR_te gtu7_process_rmc(char **tokens) {
	if(str_cmp(tokens[2], "A") == false) {
		str_cpy(internal_state.gtu7_info.time, "No data", 
			get_str_len("No data") + 1);

		str_cpy(internal_state.gtu7_info.date, "No data", 
			get_str_len("No data") + 1);

		str_cpy(internal_state.gtu7_info.lon, "No data", 
			get_str_len("No data") + 1);

		str_cpy(internal_state.gtu7_info.lat, "No data", 
			get_str_len("No data") + 1);
	...
	for(uint8_t j = 0; j < time_len; j++) {
		internal_state.gtu7_info.time[real_counter++] = tokens[1][j];

		if((real_counter + 1) % 3 == 0 && real_counter <= 5) {
			internal_state.gtu7_info.time[real_counter++] = ':';
		}
	}
	internal_state.gtu7_info.time[real_counter++] = ' ';
	internal_state.gtu7_info.time[real_counter++] = 'U';
	internal_state.gtu7_info.time[real_counter++] = 'T';
	internal_state.gtu7_info.time[real_counter++] = 'C';
	internal_state.gtu7_info.time[real_counter++] = '\0';

	// Latitude
	uint8_t lat_txt_len = get_str_len(tokens[3]);
	for(uint8_t j = 0; j < lat_txt_len; j++) {
		internal_state.gtu7_info.lat[j] = tokens[3][j];
	}
	// Add hemisphere indicators
	internal_state.gtu7_info.lat[lat_txt_len] = tokens[4][0];
	internal_state.gtu7_info.lat[lat_txt_len + 1] = '\0';

	// Longitude
	uint8_t lon_txt_len = get_str_len(tokens[5]);
	for(uint8_t j = 0; j < lon_txt_len; j++) {
		internal_state.gtu7_info.lon[j] = tokens[5][j];
	}
	// Add hemisphere indicators
	internal_state.gtu7_info.lon[lon_txt_len] = tokens[6][0];
	internal_state.gtu7_info.lon[lon_txt_len + 1] = '\0';

	// Date
	internal_state.gtu7_info.date[0] = tokens[9][0];
	internal_state.gtu7_info.date[1] = tokens[9][1];
	internal_state.gtu7_info.date[2] = '/';
	internal_state.gtu7_info.date[3] = tokens[9][2];
	internal_state.gtu7_info.date[4] = tokens[9][3];
	internal_state.gtu7_info.date[5] = '/';
	internal_state.gtu7_info.date[6] = '2';
	internal_state.gtu7_info.date[7] = '0';
	internal_state.gtu7_info.date[8] = tokens[9][4];
	internal_state.gtu7_info.date[9] = tokens[9][5];
	internal_state.gtu7_info.date[10] = '\0';
...
static ERR_te gtu7_process_vtg(char **tokens) {
	...
	if(track_made_good_len == 0) {
		str_cpy(internal_state.gtu7_info.mov_dir, "No data", 
			get_str_len("No data") + 1);
	}
	else {
		str_cpy(internal_state.gtu7_info.mov_speed, &tokens[VTG_TMG_T_DEGREE_POS][0], 
	...
	if(kph_len == 0) {
		str_cpy(internal_state.gtu7_info.mov_speed, "No data", 
			get_str_len("No data") + 1);
	}
	else {
		str_cpy(internal_state.gtu7_info.mov_speed, &tokens[VTG_SPEED_OVER_GROUND_POS][0], 
			kph_len + 1);
		internal_state.gtu7_info.mov_speed[kph_len++] = ' ';
		internal_state.gtu7_info.mov_speed[kph_len++] = 'k';
		internal_state.gtu7_info.mov_speed[kph_len++] = 'p';
		internal_state.gtu7_info.mov_speed[kph_len++] = 'h';
		internal_state.gtu7_info.mov_speed[kph_len++] = '\0';
	}
...
static ERR_te gtu7_process_gga(char **tokens) {
	...
	if(fix_status_len == 0) {
		str_cpy(internal_state.gtu7_info.fix_status, "No data",
				get_str_len("No data") + 1);
	}
	else if(str_cmp(tokens[GGA_QUALITY_FIX_POS], "0") == true) {
		char msg[] = "No valid fix";
		str_cpy(internal_state.gtu7_info.fix_status, msg, get_str_len(msg) + 1);
	}
	...
	if(num_sats_used_len == 0) {
		str_cpy(internal_state.gtu7_info.num_sats_used, "No data",
				get_str_len("No data") + 1);
	}
	else {
		str_cpy(internal_state.gtu7_info.num_sats_used, tokens[GGA_NUM_SATS_POS], 
	...
	if(ort_height_len == 0) {
		str_cpy(internal_state.gtu7_info.ort_height, "No data",
				get_str_len("No data") + 1);
	}
	else {
		str_cpy(internal_state.gtu7_info.ort_height, tokens[GGA_ORTH_HEIGHT_POS],
			ort_height_len + 1);
		internal_state.gtu7_info.ort_height[ort_height_len] = ' ';
		internal_state.gtu7_info.ort_height[ort_height_len + 1] = tokens[GGA_ORTH_HEIGHT_UNIT_POS][0];
		internal_state.gtu7_info.ort_height[ort_height_len + 2] = '\0';
	}

	// Geoid separation
	uint8_t geoid_sep_len = get_str_len(tokens[GGA_GEOID_SEP_POS]);
	if(geoid_sep_len == 0) {
		str_cpy(internal_state.gtu7_info.geoid_sep, "No data",
				get_str_len("No data") + 1);
	}
	else {
		str_cpy(internal_state.gtu7_info.geoid_sep, tokens[GGA_GEOID_SEP_POS],
				geoid_sep_len + 1);
		internal_state.gtu7_info.geoid_sep[geoid_sep_len] = ' ';
		internal_state.gtu7_info.geoid_sep[geoid_sep_len + 1] = tokens[GGA_GEOID_SEP_UNIT_POS][0];
		internal_state.gtu7_info.geoid_sep[geoid_sep_len + 2] = '\0';
	}
...
static ERR_te gtu7_process_gsa(char **tokens) {
	...
	if(msg_part_len == 0) {
		str_cpy(internal_state.gtu7_info.fix_type, "No data",
		get_str_len("No data") + 1);
	}
	else {
		if(str_cmp(tokens[GSA_FIX_TYPE_POS], "1") == true) {
			str_cpy(internal_state.gtu7_info.fix_type, "Not available",
			get_str_len("Not available") + 1);
		}
		else if(str_cmp(tokens[GSA_FIX_TYPE_POS], "2") == true) {
			str_cpy(internal_state.gtu7_info.fix_type, "2D",
			get_str_len("2D") + 1);
		}
		else if(str_cmp(tokens[GSA_FIX_TYPE_POS], "3") == true) {
			str_cpy(internal_state.gtu7_info.fix_type, "3D",
			get_str_len("3D") + 1);
		}
	}
	...
	if(msg_part_len == 0) {
		str_cpy(internal_state.gtu7_info.pdop, "No data",
		get_str_len("No data") + 1);
	}
	else {
		str_cpy(internal_state.gtu7_info.pdop, tokens[GSA_PDOP_POS],
		msg_part_len + 1);
	}

	msg_part_len = get_str_len(tokens[GSA_HDOP_POS]);
	if(msg_part_len == 0) {
		str_cpy(internal_state.gtu7_info.hdop, "No data",
		get_str_len("No data") + 1);
	}
	else {
		str_cpy(internal_state.gtu7_info.hdop, tokens[GSA_HDOP_POS],
		msg_part_len + 1);
	}
	...
	if(msg_part_len == 0) {
		...
		str_cpy(internal_state.gtu7_info.vdop, tokens[GSA_VDOP_POS],
		msg_part_len);
		internal_state.gtu7_info.vdop[msg_part_len] = '\0';
	}
...
static ERR_te gtu7_process_gsv(char **tokens) {
	...
	if(msg_part_len == 0) {
		str_cpy(internal_state.gtu7_info.num_sats_all, "No data",
		get_str_len("No data") + 1);		
	}
	else {
		str_cpy(internal_state.gtu7_info.num_sats_all, tokens[GSV_NUM_SATS_VISIBLE_POS],
		msg_part_len + 1);
	}
...
static ERR_te gtu7_dumpnmea_handler(uint32_t argc, char **argv) {
	if(argc != 3) {
		LOG_ERROR(
			internal_state.subsys,
			internal_state.log_level,
			"gtu7_dumpnmea_handler: invalid arguments"
		);
